 ...
 * Image Input Example
 * 
 * Demonstrates multimodal image analysis with the DashScope API.
 * This example sends multiple images along with a text prompt
 * to the Qwen VL Plus model for visual analysis.
 */
...
int main(int argc, char **argv)
{
	...
	DS_Content user_contents[] = {
		ds_content_text("Please describe these images:"),
		ds_content_image("https://dashscope.oss-cn-beijing.aliyuncs.com/images/dog_and_girl.jpeg"),
		ds_content_image("https://dashscope.oss-cn-beijing.aliyuncs.com/images/tiger.png"),
		ds_content_image("https://dashscope.oss-cn-beijing.aliyuncs.com/images/rabbit.png")
	};
	
	DS_Message messages[] = {
		{ .role = DS_MESSAGE_ROLE_SYSTEM, .content = &system_prompt, .content_count = 1 },
		{ .role = DS_MESSAGE_ROLE_USER, .content = user_contents, .content_count = 4 }
	};
